...
export default function Home() {
  ...
  const handleUpload = async () => {
    try {
      ...
      // Compression côté navigateur (évite 413 / Request Entity Too Large)
      ...
      // Essaye JSON, sinon affiche le texte brut
      let json = null;
      try {
      ...
      if (!response.ok) {
        const msg =
          ...
          text ||
          `Erreur serveur (status ${response.status})`;
        setError(msg);
        setLoading(false);
        return;
      }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f6fb", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ width: "100%", maxWidth: 640, background: "white", borderRadius: 14, padding: 28, boxShadow: "0 10px 30px rgba(0,0,0,0.08)" }}>
