...
export function buildWorldPulseBrief({ articles = [], countrySignals = [], offMapArticles = [] } = {}) {
  ...
  const localizationPct = safeArticles.length > 0 ? Math.round((localizedArticles / safeArticles.length) * 100) : 0;
  ...
  return {
    ...
    concentrationPct,
    headline,
    methodNote: "Ce brief décrit les articles RSS réellement reçus et les pays explicitement détectés dans leur titre ou résumé. Il ne mesure pas l'intensité réelle des événements.",
  };
}
