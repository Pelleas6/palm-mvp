...
function filterOptions(articles) {
  return {
    ...
    countries: countBy(articles, sourceCountryKey, (_value, article) => ({
      label: sourceCountryLabel(article),
      code: sourceLocationForArticle(article)?.code || null,
      verified: Boolean(sourceLocationForArticle(article)?.code),
...
export function articleMatchesWorldPulseFilters(article, filters = {}) {
  ...
  if (normalized.country !== WORLD_PULSE_FILTER_ALL) {
    const location = sourceLocationForArticle(article);
    const countryValue = location?.code || sourceCountryLabel(article);
    if (countryValue !== normalized.country && sourceCountryLabel(article) !== normalized.country) return false;
  }
...
function buildCounts(articles, particles, clusters, offMapArticles) {
  return {
    ...
    countries: distinctCount(articles, sourceCountryKey),
...
function buildGroupings(articles, offMapArticles) {
  return {
    ...
    locations: countBy(articles.filter((article) => sourceLocationForArticle(article)?.code), sourceCountryKey, (_value, article) => ({
      label: sourceCountryLabel(article),
      code: sourceLocationForArticle(article)?.code || null,
    })).map((item) => ({ ...item, value: item.code || item.value })),
...
function selectionArticles(payload, filteredArticles, selection, derived) {
  ...
  if (selection.type === "country") {
    ...
    return filteredArticles.filter((article) => sourceLocationForArticle(article)?.code === code);
  }
...
function selectionLabel(selection, articles) {
  if (selection?.type === "country") return `Pays média : ${sourceCountryLabel(articles[0])}`;
  if (selection?.type === "marker") return `Repère média : ${mediaNameForArticle(articles[0])}`;
  if (selection?.type === "cluster") return `Cluster articles : ${sourceCountryLabel(articles[0])}`;
...
function buildSelectionSummary(selection, articles) {
  ...
  return {
    ...
    isoVerified: articles.every((article) => Boolean(sourceLocationForArticle(article)?.code)),
    basis: "Lecture fondée sur le pays média ISO vérifié ; aucune géolocalisation d'événement n'est déduite.",
  };
}
...
export function deriveWorldPulseExploration(payload = {}, filters = {}, selection = null) {
  ...
  return {
    ...
    availableCountryCodes: [...new Set(filteredArticles.map((article) => sourceLocationForArticle(article)?.code).filter(Boolean))],
