...
// --- PROMPT SYSTÈME ---
// Note : Les consignes imposent un format Markdown strict, 7 sections,
// et une profondeur minimale de 600 mots. Tout changement ici 
// doit être testé avec le modèle vision.
export function buildSystemPrompt() {
  return `Tu es un analyste spécialisé dans l’étude des mains, formes et reliefs.
...
- Produis un rapport détaillé d'au moins 600 mots. Chaque section doit être approfondie avec des observations précises.
- Si le contenu te semble incomplet, prends le temps de développer davantage avant de finaliser.
- Si tu detectes des incohérences, mentionne-les dans la synthèse.
...
export async function generateReport(supabase, geminiModel, prenom, dateNaissance, themeChoisi, leftPath, rightPath) {
  ...
  const leftBase64 = Buffer.from(await leftData.arrayBuffer()).toString("base64");
  const rightBase64 = Buffer.from(await rightData.arrayBuffer()).toString("base64");

  const prompt = `${buildSystemPrompt()}\n\n${buildUserPrompt(prenom, dateNaissance, themeChoisi)}`;

  const result = await geminiModel.generateContent([
    prompt,
    { inlineData: { data: leftBase64, mimeType: leftMime } },
    { inlineData: { data: rightBase64, mimeType: rightMime } },
  ]);

// Cycle 43: Amélioration du système de log pour le déploiement Vercel
  const reportText = result.response.text();
  if (!reportText || !reportSchema.validate(reportText)) {
    console.error("DEBUG - Rapport IA invalide reçu:", reportText);
    throw new Error("Gemini returned invalid or incomplete report");
  }
  return reportText;
}
