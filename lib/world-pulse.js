...
function buildMapPoints(articles) {
  ...
  for (const entries of byLocation.values()) {
    ...
    entries.forEach((entry, index) => {
      ...
      const radius = count <= 1 ? 0 : Math.min(4.8, 1.4 + count * 0.32);
      const x = clamp(entry.location.x + Math.cos(angle) * radius, 4, 96);
      const y = clamp(entry.location.y + Math.sin(angle) * radius * 0.72, 8, 88);
      ...
      points.push({
        ...
        articleCount,
        size: Math.round(14 + Math.min(24, Math.sqrt(articleCount) * 6)),
        ...
        positioning: "media_source_location",
      });
    });
  }
...
function buildDataPayload({ state, stateLabel, generatedAt, source, query, articles, notice, cacheTtlMs, cacheStatus = "miss", sourceHealth = [] }) {
  ...
  const mapPoints = buildMapPoints(articles);
