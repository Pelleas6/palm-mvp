...
const GDELT_CANARY_MAX_RECORDS = 25;
...
function classifyMediaSignal(...parts) {
  ...
  return {
    label: signal.label,
    labelType: classified ? "registre déterministe" : "à classifier",
    labelBasis: classified ? "mots-clés du registre dans le titre/description/source" : "aucun mot-clé du registre détecté",
    classified,
  };
}
...
function buildOffMapArticle(article, reason = "source_country_iso_unverified", label = "Pays média source non vérifié", detail = null) {
  return {
    ...
    reason,
    reasonLabel: label,
    detail: detail || `${label} : ${article?.sourceCountry || "Non précisé"}. Aucune position n'est inventée.`,
  };
}
...
function buildArticleParticle(article) {
  ...
  const seed = `${article.id || article.url || article.title}:${location.code}:article-particle`;
  const placement = placePointInsideCountry(location.code, seed);
  if (!placement) {
    return {
      offMap: buildOffMapArticle(
        article,
        "country_geometry_position_failed",
        "Placement dans géométrie pays impossible",
        `Pays média source ${location.label} vérifié, mais aucun point interne n'a pu être calculé dans la géométrie embarquée.`
      ),
    };
  }
  ...
  for (const bucket of buckets.values()) {
    ...
    for (const cluster of localClusters.filter((item) => item.entries.length > 1)) {
      ...
      clusters.push({
        ...
        size: clamp(Math.round(9 + Math.sqrt(entries.length) * 3), 11, 26),
        ...
        articles: entries.map((entry) => ({ id: entry.id, title: entry.title, url: entry.url, mediaName: entry.mediaName })),
        positioning: {
          basis: "visual_cluster_same_country_category",
          sourceCountryIso2: first.location?.code || null,
...
function buildWebNgramsTrends(entries, { timestamp, url, checkedAt = null, state = "OK", stale = false, error = null, toc = null } = {}) {
  ...
  const classification = buildClassificationSummary(documents);
  const labels = classification.categories;
  ...
  return {
    ...
    labels,
    categories: labels,
    thematicCategories: classification.thematicCategories,
    classification: {
...
async function fetchWebNgramsTocAttempt({ fetchImpl, timeoutMs, referenceDate }) {
...
async function fetchWebNgramsToc({ cache, fetchImpl, timeoutMs, referenceDate }) {
  ...
  ngramsCache.pending = fetchWebNgramsTocAttempt({ fetchImpl, timeoutMs, referenceDate })
    .then((result) => {
...
function buildDataPayload({ state, stateLabel, generatedAt, source, query, articles, notice, cacheTtlMs, cacheStatus = "miss", sourceHealth = [], globalTrends = emptyWebNgramsTrends(), rssStats = {} }) {
  ...
  return {
    ...
    groupings: {
      ...
      gdeltNgramsCategories: gdeltCategories,
      offMapReasons,
    },
    dataScopes: {
      rss: rssScope,
