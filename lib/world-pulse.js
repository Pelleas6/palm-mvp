...
const DEFAULT_RSS_FEEDS = [
  { name: "BBC News World", url: "https://feeds.bbci.co.uk/news/world/rss.xml", language: "English", sourceCountry: "United Kingdom" },
  { name: "France 24 Monde", url: "https://www.france24.com/fr/rss", language: "French", sourceCountry: "France" },
  { name: "NPR World", url: "https://feeds.npr.org/1004/rss.xml", language: "English", sourceCountry: "United States" },
];
...
function parseRssFeedOverrides() {
  ...
  return raw
    ...
    .filter(Boolean)
    .map((entry) => {
      const [name, url, sourceCountry, language] = entry.includes("|")
      ...
      return {
        ...
        sourceCountry: trimText(sourceCountry, 80) || "Non précisé",
      };
    })
    .filter(Boolean);
}
...
async function fetchRssFallback({ fetchImpl, timeoutMs, feeds }) {
  ...
  const okFeeds = results
    ...
    .map((result) => ({ name: result.feed.name, url: result.feed.url, sourceCountry: result.feed.sourceCountry || "Non précisé", status: result.status }));
