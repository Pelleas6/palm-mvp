...
import frenchRssFeedCatalog from "../content/rss-feeds-france.json" with { type: "json" };
...
import {
  placePointInsideAdministrativeArea,
  placePointInsideCountry,
  resolveVerifiedAdministrativeArea,
...
const MAX_HTTP_RESPONSE_BYTES = 3_000_000;
...
const FRENCH_RSS_FEEDS = frenchRssFeedCatalog
  .filter((feed) => feed?.enabled === true)
  .map((feed) => ({
    name: `${feed.media} — ${feed.feed}`,
    region: feed.region || "Europe",
    url: feed.url,
    language: feed.language || "French",
    sourceCountry: feed.sourceCountry || "France",
...
export const DEFAULT_RSS_FEEDS = [
  ...
  // RSF complète les espaces peu couverts par des rédactions locales dans notre sélection.
  // Les flux sont plafonnés et disposent d'un délai borné adapté à leur serveur dynamique.
  { name: "RSF — Afrique", region: "Africa", url: "https://rsf.org/fr/rss/afrique/feed.xml", language: "French", sourceCountry: "France", maxItems: 10, timeoutMs: 8000 },
  ...
  { name: "RSF — Maghreb / Moyen-Orient", region: "Middle East", url: "https://rsf.org/fr/rss/maghreb-moyen-orient/feed.xml", language: "French", sourceCountry: "France", maxItems: 10, timeoutMs: 8000 },
  ...
  { name: "RSF — Asie-Pacifique", region: "Asia", url: "https://rsf.org/fr/rss/asie-pacifique/feed.xml", language: "French", sourceCountry: "France", maxItems: 10, timeoutMs: 8000 },
  ...
  { name: "RSF — Argentine", region: "South America", url: "https://rsf.org/fr/rss/am%C3%A9riques/argentine/feed.xml", language: "French", sourceCountry: "France", maxItems: 6, timeoutMs: 8000 },
  { name: "RSF — Haïti", region: "North America", url: "https://rsf.org/fr/rss/am%C3%A9riques/ha%C3%AFti/feed.xml", language: "French", sourceCountry: "France", maxItems: 6, timeoutMs: 8000 },
  { name: "RSF — Venezuela", region: "South America", url: "https://rsf.org/fr/rss/am%C3%A9riques/venezuela/feed.xml", language: "French", sourceCountry: "France", maxItems: 6, timeoutMs: 8000 },
...
function eventLocationForArticle(article) {
  ...
  const administrativeArea = resolveVerifiedAdministrativeArea(
    location.code,
    article.eventAdministrativeAreaCode,
  );
  return {
    ...location,
...
function buildOffMapArticle(article, reason = "event_country_not_detected", label = "Événement non localisé", detail = null) {
  return {
    ...
    eventAdministrativeArea: article?.eventAdministrativeArea || null,
    eventAdministrativeAreaCode: article?.eventAdministrativeAreaCode || null,
...
function buildArticleParticle(article) {
  ...
  const placement = location.administrativeArea
    ? placePointInsideAdministrativeArea(location.code, location.administrativeArea.code, seed)
    : placePointInsideCountry(location.code, seed);
  if (!placement) {
    return {
  ...
  return {
    particle: {
      ...
      positioning: {
        ...placement.positioning,
        basis: location.administrativeArea
          ? "verified_event_administrative_area_geometry"
          : "verified_event_country_geometry",
        eventCountryIso2: location.code,
        eventCountryLabel: location.label,
        ...(location.administrativeArea ? {
          eventAdministrativeAreaCode: location.administrativeArea.code,
          eventAdministrativeAreaLabel: location.administrativeArea.label,
        } : {}),
      },
    },
  };
  ...
  for (const bucket of buckets.values()) {
    ...
    for (const cluster of localClusters.filter((item) => item.entries.length > 1)) {
      ...
      clusters.push({
        ...
        eventAdministrativeArea: first.eventAdministrativeArea || null,
        eventAdministrativeAreaCode: first.eventAdministrativeAreaCode || null,
...
function joinDistinctText(...values) {
  return [...new Set(values.map((value) => stripTags(decodeXmlEntities(value))).filter(Boolean))].join(" ");
}
...
function parseRssItems(xml, feed) {
  ...
  return entries.map((block, index) => {
    ...
    const description = joinDistinctText(extractTag(block, "description"), extractTag(block, "summary"));
    // RSS emploie souvent content:encoded, tandis qu'Atom utilise content.
    const rssContent = joinDistinctText(extractTag(block, "content:encoded"), extractTag(block, "content"));
    const signalText = joinDistinctText(description, rssContent);
    ...
    const signal = classifyMediaSignal(title, signalText);
    const eventLocation = resolveEventCountryFromArticle({ title, summary: description, content: rssContent });
    return {
      ...
      summary: trimText(description || rssContent, 260),
      ...eventLocation,
      ...signal,
    };
  }).filter(Boolean);
...
async function fetchSingleRssFeed(feed, { fetchImpl, timeoutMs, referenceDate }) {
  try {
    const sourceTimeoutMs = clamp(Number(feed?.timeoutMs) || timeoutMs, 1000, 15000);
...
function buildDataPayload({ state, stateLabel, generatedAt, source, query, articles, notice, cacheTtlMs, cacheStatus = "miss", sourceHealth = [], globalTrends = emptyWebNgramsTrends(), rssStats = {} }) {
  ...
  // La localisation, les bulles et les éléments de détail sont déjà calculés
  // côté serveur. Le résumé RSS et l'image ne sont jamais lus par le client :
  // ne pas les sérialiser évite de faire voyager plusieurs centaines de textes
  // supplémentaires à chaque snapshot.
  const dashboardArticles = articles.map(({ summary: _summary, image: _image, ...article }) => article);
...
// Le modèle interne conserve les preuves et la géométrie complète afin que les
// contrôles serveur restent auditables. Le tableau de bord, lui, ne consomme
// qu'un sous-ensemble précis de ces propriétés : cette projection retire les
// doublons avant sérialisation dans le HTML et dans /api/gdelt.
function compactDashboardArticle(article) {
  return {
    id: article?.id || null,
    title: article?.title || "Article sans titre",
    url: article?.url || null,
    domain: article?.domain || null,
    mediaName: article?.mediaName || null,
    sourceCountry: article?.sourceCountry || null,
    sourceRegion: article?.sourceRegion || null,
    sourceLocation: article?.sourceLocation || null,
    ...
    labelType: article?.labelType || null,
    confidence: Number(article?.confidence || 0),
  };
}

function compactDashboardParticle(particle) {
  return {
    id: particle?.id || null,
    articleId: particle?.articleId || null,
    mediaName: particle?.mediaName || null,
    sourceCountry: particle?.sourceCountry || null,
    eventCountry: particle?.eventCountry || null,
    eventAdministrativeArea: particle?.eventAdministrativeArea || null,
    eventAdministrativeAreaCode: particle?.eventAdministrativeAreaCode || null,
    location: particle?.location || null,
    x: particle?.x || 0,
    y: particle?.y || 0,
    size: particle?.size || ARTICLE_PARTICLE_MIN_SIZE,
    label: particle?.label || WORLD_PULSE_UNCLASSIFIED_LABEL,
    ...(particle?.clusterId ? { clusterId: particle.clusterId } : {}),
  };
}

function compactDashboardCluster(cluster) {
  return {
    id: cluster?.id || null,
    label: cluster?.label || WORLD_PULSE_UNCLASSIFIED_LABEL,
    location: cluster?.location || null,
    sourceCountry: cluster?.sourceCountry || null,
    eventCountry: cluster?.eventCountry || null,
    eventAdministrativeArea: cluster?.eventAdministrativeArea || null,
    eventAdministrativeAreaCode: cluster?.eventAdministrativeAreaCode || null,
    x: cluster?.x || 0,
    y: cluster?.y || 0,
    size: cluster?.size || 22,
    count: Number(cluster?.count || 0),
    articleCount: Number(cluster?.articleCount || cluster?.count || 0),
    mediaNames: Array.isArray(cluster?.mediaNames) ? cluster.mediaNames : [],
    articles: Array.isArray(cluster?.articles)
      ? cluster.articles.map((article) => ({ id: article?.id || null, articleId: article?.articleId || null }))
      : [],
  };
}

function compactDashboardMarker(marker) {
  return {
    id: marker?.id || null,
    mediaName: marker?.mediaName || null,
    sourceCountry: marker?.sourceCountry || null,
    location: marker?.location || null,
    x: marker?.x || 0,
    y: marker?.y || 0,
    size: marker?.size || MEDIA_MARKER_MIN_SIZE,
    label: marker?.label || WORLD_PULSE_UNCLASSIFIED_LABEL,
    articleCount: Number(marker?.articleCount || 0),
  };
}

function compactDashboardOffMapArticle(article) {
  return {
    id: article?.id || null,
    reason: article?.reason || "event_country_not_detected",
    reasonLabel: article?.reasonLabel || "Événement non localisé",
  };
}

function compactDashboardPayload(payload) {
  const { mapPoints: _mapPoints, ...dashboard } = payload;
  return {
    ...dashboard,
    articles: Array.isArray(payload?.articles) ? payload.articles.map(compactDashboardArticle) : [],
    mediaMarkers: Array.isArray(payload?.mediaMarkers) ? payload.mediaMarkers.map(compactDashboardMarker) : [],
    articleParticles: Array.isArray(payload?.articleParticles) ? payload.articleParticles.map(compactDashboardParticle) : [],
    articleClusters: Array.isArray(payload?.articleClusters) ? payload.articleClusters.map(compactDashboardCluster) : [],
    offMapArticles: Array.isArray(payload?.offMapArticles) ? payload.offMapArticles.map(compactDashboardOffMapArticle) : [],
  };
}
...
export async function getWorldPulseDashboardPayload(options = {}) {
  return compactDashboardPayload(assertDashboardPayload(await getWorldPulse(options)));
}
