...
// ...
export async function processAnalyzePayload(body, { geminiModel, supabase, resend } = {}) {
  ...
  const resendClient = resend || getResendClient();
  ...
  const geminiModel = geminiModel || getGeminiClient();

  try {
    const report = await generateReport(supabaseClient, geminiModel, prenom, dateNaissance, themeChoisi, leftPath, rightPath);
    await sendEmails(resendClient, prenom, nom, email, dateNaissance, themeChoisi, report);
  } catch (e) {
