...
test("dashboard payload contract keeps API, server page and component on the same validated object", async () => {
  ...
  assert.match(componentSource, /Chaque repère représente un pays ou une zone de pays voisins/);
  ...
  assert.match(componentSource, /function MapCountryPicker/);
  assert.match(componentSource, /function SituationBrief/);
  assert.match(componentSource, /country-beacon/);
